...
The independently earned permission prerequisite for broad automation of one versioned Plus, x5, or x20 checkout path and PHP price contract, established by one complete live canary whose card payment settles, target membership confirms, renewal is disabled, and exercised adapter path has no unresolved outcome. Plus qualification is the shared prerequisite for starting either staged-upgrade canary; x5 and x20 then branch independently because each performs its own Plus payment followed by its target-specific upgrade. Qualification never enables a scope by itself or carries into another tier or contract version.
_Avoid_: Plus qualification standing in for a completed x5/x20 canary, requiring x5 before x20, pending authorization as rollout proof, automatic promotion, reusing evidence across versions
