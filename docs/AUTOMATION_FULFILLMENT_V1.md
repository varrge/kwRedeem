...
保存不可变映射快照 + clientOrderId
        |
        v
从卡台实时卡列表选择适用卡片、按需写入本地预留与对账事实、必要时开卡或充值
        |
        v
...
如果第一次 attempt 已经为卡片建立资金意图，后续映射只能复用同一卡台、容量分组和已预留卡，且整卡预存金额不能更高，避免为同一订单建立第二条资金边界。SpaceX Card 已有卡可使用当前目录中任一明确支持 GPT 的产品；映射里的产品代码仅作为无可用卡时的首选开卡产品，失效时从当前合格产品中确定性选择费用最低者。
...
SpaceX Card 协议自动化不要求管理员先初始化库存。每笔真实订单需要卡片时直接读取卡台完整实时列表与当前产品目录，只考虑 `ACTIVE` 且明确允许 GPT 的卡；再读取候选卡交易记录，计算套餐 lane 与已用容量。只有经过判定并实际参与选择的卡才最小化写入 `managed_cards`，用于本地唯一槽位预留、资金幂等和后续对账。后台的全量库存任务仅保留给旧会员履约和历史卡审计，不再是协议自动化前置条件。
