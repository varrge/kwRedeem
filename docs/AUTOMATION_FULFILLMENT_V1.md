...
SpaceX GPT 在创建远端订单前明确返回“该卡直充额度已用满、请换卡”时，表示远端任务未创建。系统把旧卡标记为容量已满，并直接为同一订单开一张按映射完整预存的新卡后重新提交。旧卡已经成功发生的充值仍作为第一条不可变资金意图保留，新卡开卡使用第二条独立幂等资金意图；任一资金结果不明确或替换卡再次被拒时转人工核验，不连续开卡。
