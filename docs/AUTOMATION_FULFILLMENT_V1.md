...
后台映射只能选择站点最近一次 `/config` 返回的 `taskType=purchase` 套餐和地区。能力被站点移除、任务类型改变或地区币种变化时，相关映射自动停用。
...
如果第一次 attempt 已经为卡片建立资金意图，后续映射只能复用同一卡台、容量分组、卡产品，且备付金额不能更高，避免为同一订单建立第二条资金边界。
