...
整卡预存金额按容量分摊为订单槽位风险。Plus 配置为 82 USD、容量 5 时，每个订单占用 16.40 USD 日风险；开新卡只在第一个槽位一次预存 82 USD。复用已有卡时只检查卡内余额能否覆盖当前一个槽位，不再要求补足所有剩余槽位的资金池；余额不足时只充值一次单槽额度，并按卡产品最低充值额向上取整。例如卡产品最低充值 20 USD 时，本次只充值 20 USD，不会补到剩余池目标。x5/x20 配置容量 1 时，每个订单独占整张卡，槽位风险等于整卡预存金额。

SpaceX GPT 在创建远端订单前明确返回“该卡直充额度已用满、请换卡”或“所选卡片当前不可支付”时，表示远端任务未创建且当前卡不可继续使用。系统把旧卡标记为容量已满，并直接为同一订单开一张按映射完整预存的新卡后重新提交。旧卡已经成功发生的充值仍作为第一条不可变资金意图保留，新卡开卡使用第二条独立幂等资金意图；任一资金结果不明确或替换卡再次被拒时转人工核验，不连续开卡。

## API Key

API Key 属于站点实例，不属于订单。管理员首次配置时提供一次，之后所有新订单复用当前 Key；编辑站点时留空表示继续使用。Automate 使用 `X-Automate-Key`，eFun 使用 `X-API-Key`，两者都只在服务端发送。

站点能力在管理员保存站点时同步一次；之后仅在真实用户订单首次进入路由时，按该商品关联的启用站点检查一次。空闲期间不做固定周期轮询，同一订单等待映射时也不会重复请求能力接口。一次能力检查失败只影响该站点本次路由；后续新订单可重新探测，若先前熔断来自能力同步失败，探测成功后自动恢复。
...
远端已经确认会员开通、仅等待关闭自动续费时，订单继续轮询但不占用站点开通并发。超过 30 分钟仍明确开启自动续费的订单转入 `manual_review`，由管理员关闭续费并依据外部证据裁决；该订单仍保留卡片和资金边界，不能提前判成功。
