...
- A non-manual membership product has an explicit `membership_tier` of `plus`, `x5`, or `x20`. A manual-processing membership CDK instead carries the explicit purchased tier in its stored `manual_type` (`PLUS`, `x5`, or `x20`). The order and fulfillment snapshot the applicable explicit value at creation. Titles, prefixes, and site slugs are never tier authorities.
...
When a redeem order uses a product with a configured membership tier, or an explicit manual-processing membership CDK, the order transaction creates one Membership Fulfillment in `WAITING_SESSION_ACTIVATION` and snapshots the target tier. Manual `PLUS` normalizes to `plus`; `x5` and `x20` remain unchanged. This does not change the existing order or activation-job status. A single-order audited backfill may repair a historical order only after its Cookie delivery is already `succeeded`; there is no bulk historical backfill.
...
- For a pre-automation card manually used for a Plus CDK, expose an audited administrator action only while the active card is unassigned and held for `PENDING_SETTLEMENT`. Re-run classification against the stored normalized transaction history with a Plus lane constraint; require at least one recognized pending OpenAI authorization and reject mixed-tier, unknown-price, or over-capacity evidence. This action updates only local lane/capacity/reconciliation fields and never calls an upstream card write API.
...
- Eligible new redeem orders now create exactly one Membership Fulfillment with a snapshotted explicit target tier from the manual membership CDK or non-manual product. Successful Session Activation Delivery activates a keyed, non-reversible account lock; duplicate ChatGPT identities wait behind the current holder instead of overlapping card or browser work.
