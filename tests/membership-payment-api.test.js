...
test("rollout gate accepts a ready EfunCard platform and starts one self-contained x20 canary", async () => {
  db.prepare(`
  ...
  `).run(fixedNow, fixedNow);
  assert.equal(db.prepare("SELECT COUNT(*) AS count FROM tier_rollout_qualifications").get().count, 0);
