...
KaWang uses this artwork as the Sentinel Boss. The character artwork and animation are by LudicArts. KaWang's changes are registration alignment, shared cropping/resizing, and WebP atlas packing; no recoloring or generated artwork is included.

## Derived files

All source PNGs retain transparency and original color. The source provides a different fixed canvas for each action. A single fixed translation per action aligns its initial standing pose to the initial idle pose; that translation is applied identically to every frame of that action. Relative movement inside each action is preserved. The union of all selected translated alpha bounds is padded by eight source pixels and cropped to `(-224, -75, 549, 582)`, then resized uniformly to **512 × 435**. There is no per-frame trimming or recentering. Registration translations and original canvas dimensions are in `metadata.json`.
...
Source directory: `ForestFiendsPackOGA/_walking_tree_pack/Large/`

| Output | Source frames | Frames | Duration |
| --- | --- | ---: | ---: |
| `idle.webp` | `Idle1__000.png` through `__009.png` | 10 | 833 ms |
| `idle-battle.webp` | `Idle2__000.png` through `__009.png` | 10 | 833 ms |
| `hurt.webp` | `Hurt__000.png` through `__009.png` | 10 | 833 ms |
| `death.webp` | `Death2__000.png` through `__009.png` | 10 | 833 ms |
